document.addEventListener("DOMContentLoaded", () => {
    ...
    function renderNavbar() {
        ...
        if (user) {
            if (user.role === 'Admin') {
                ...
                linksHtml = `
                    ...
                    <a href="javascript:navigate('trainer_schedule')" id="nav-trainer_schedule">Kelola Jadwal Pribadi</a>
                    <a href="javascript:navigate('about')" id="nav-about">Tentang</a>
                `;
            } else { // Client
                linksHtml = `
        ...
        // --- KODE GABUNGAN YANG SUDAH DIPINDAHKAN KE POSISI YANG BENAR ---
        // Menutup menu saat pengguna mengklik di luar area menu pada tampilan mobile
        document.addEventListener('click', (event) => {
            const isClickInsideMenu = navLinks.contains(event.target);
            const isClickOnToggler = navToggle.contains(event.target);

            if (navLinks.classList.contains('active') && !isClickInsideMenu && !isClickOnToggler) {
                ...
                fetch('logout.php', { method: 'POST', headers: {'Content-Type':'application/json'} })
                    ...
                    .then(result => {
                        if (result.success) {
                            ...
                            sessionStorage.setItem('toastMessage', 'Logout berhasil.');
    ...
    window.navigate = function (page, params = {}) {
        ...
        switch (page) {
            ...
            case "trainer_schedule": renderTrainerScheduleManagePage(); break;
    ...
    // NEW: Function to toggle schedule visibility
    window.toggleSchedule = function(trainerId) {
        const scheduleDiv = document.getElementById(`schedule-for-trainer-${trainerId}`);
        if (scheduleDiv) {
...
// =========================================================================
    // == HALAMAN TRAINER GYM (GABUNGAN PILIH TRAINER & JADWAL KETERSEDIAAN) ==
    // =========================================================================
    function renderTrainerPage() {
        const user = getLoggedInUser();
        contentContainer.innerHTML = `<p>Memuat data trainer dan jadwal...</p>`;

        // Fetch both users (for trainer details including contact and profile) and schedules
        const usersPromise = fetch("get_users.php").then((response) => response.json());
        const schedulesPromise = fetch('get_trainer_availability.php').then(response => response.json());

        Promise.all([usersPromise, schedulesPromise])
            .then(([usersData, scheduleData]) => {
                
                let trainerContentHtml = '';

                if (usersData.success && usersData.users) {
                    const trainers = usersData.users.filter((u) => u.role === "Trainer");
                    
                    // Jika user adalah client dan sudah punya trainer
                    if (user && user.role === 'Client' && user.trainer && user.trainer.id) {
                        const chosenTrainer = trainers.find((t) => t.id == user.trainer.id);
                        if (chosenTrainer) {
                            // Ensure chosenTrainer.profile and chosenTrainer.contact are available
                            // Use a default image if profile is not set or invalid
                            const trainerProfilePath = chosenTrainer.profile && chosenTrainer.profile !== '' ? `./${chosenTrainer.profile}` : `./PIC/${chosenTrainer.gender === "Laki-laki" ? "pria.jpg" : "wanita.jpg"}`;
                            const whatsappLink = chosenTrainer.contact ? `https://wa.me/${chosenTrainer.contact}?text=Halo%20${encodeURIComponent(chosenTrainer.fullName)},%20saya%20ingin%20melakukan%20konsultasi.` : '#';

                            trainerContentHtml = `
                                <div class="trainer-chosen-section">
                                    <header><h1>Trainer Anda</h1></header>
                                    <div class="trainer-card">
                                        <div class="trainer-card-main">
                                            <div class="trainer-details">
                                                <img src="${trainerProfilePath}" alt="${chosenTrainer.fullName}">
                                            ...
                                            <div class="trainer-actions">
                                                <button type="button" class="action-btn" onclick="window.open('${whatsappLink}', '_blank')">Hubungi ${chosenTrainer.fullName}</button>
                    ...
                    } else { // Tampilan standar untuk memilih trainer
                        let trainerCards = "";
                        trainers.forEach((trainer) => {
                            // Determine profile image path
                            // Use a default image if profile is not set or invalid
                            const trainerProfilePath = trainer.profile && trainer.profile !== '' ? `./${trainer.profile}` : `./PIC/${trainer.gender === "Laki-laki" ? "pria.jpg" : "wanita.jpg"}`;
                            // Create WhatsApp link using trainer's contact
                            const whatsappLink = trainer.contact ? `https://wa.me/${trainer.contact}?text=Halo%20${encodeURIComponent(trainer.fullName)},%20saya%20ingin%20melakukan%20konsultasi.` : '#';

                            // Find schedules for the current trainer
                            const trainerSchedules = scheduleData.success ? (scheduleData.schedules[trainer.fullName] || []) : [];
                            let scheduleHtml = '';
                            ...
                            if (trainerSchedules.length > 0) {
                                ...
                                scheduleHtml = `
                                <div class="trainer-schedule-details" id="schedule-for-trainer-${trainer.id}">
                                    <h4>Jadwal Trainer</h4>
                                    <table class="schedule-table">
                                        <thead>
                                            <tr><th>Hari</th><th>Waktu Mulai</th><th>Waktu Selesai</th><th>Deskripsi</th></tr>
                                        </thead>
                            ...
                            trainerCards += `
                                <div class="trainer-card" data-name="${trainer.fullName.toLowerCase()}">
                                    <div class="trainer-card-main">
                                        <div class="trainer-details">
                                            <img src="${trainerProfilePath}" alt="${trainer.fullName}">
                                            <div class="trainer-info">
                                        ...
                                        <div class="trainer-actions">
                                            ...
                                            ${scheduleHtml ? `<button type="button" class="view-schedule-btn" onclick="toggleSchedule(${trainer.id})">Lihat Jadwal</button>` : ''}
                                        </div>
                                    </div>
                                    ${scheduleHtml}
                                </div>`;
                        ...
                        trainerContentHtml = `
                            <div class="trainer-section">
                                <header><h1>Pilih Trainer Anda</h1></header>
                ...
                contentContainer.innerHTML = trainerContentHtml;

                // --- Inisialisasi Event Listener ---
                const searchTrainerInput = document.getElementById('searchTrainerInput');
                if(searchTrainerInput){
                    searchTrainerInput.addEventListener('keyup', function() {
                ...
                document.querySelectorAll(".hire-btn").forEach((button) => {
                    button.addEventListener("click", () => {
                        ...
                        if (loggedInUser && loggedInUser.role === 'Client') {
                           ...
                           showToast('Hanya Client yang dapat memilih trainer.', 'warning');
                        }
                    });
                });
            })
            .catch((error) => {
                console.error("Error:", error);
                contentContainer.innerHTML = "<p>Terjadi kesalahan saat memuat data halaman.</p>";
            });
    }
    ...
    // =========================================================================
    // == IMPLEMENTASI JADWAL KETERSEDIAAN TRAINER (TRAINER SCHEDULE MANAGE) ==
    // =========================================================================
    function renderTrainerScheduleManagePage() {
        const user = getLoggedInUser();
        if (!user || user.role !== 'Trainer') {
            contentContainer.innerHTML = `<p>Hanya trainer yang dapat mengakses halaman ini.</p>`;
            return;
        ...
        contentContainer.innerHTML = `<p>Memuat jadwal ketersediaan Anda...</p>`;
    ...
    window.updateDashboard = function (event) {
        ...
        if (user) {
            ...
            // Hanya ambil file jika elemen input gambar terlihat (yaitu, untuk Trainer)
            const file = (user.role === 'Trainer' && inputImage) ? inputImage.files[0] : null;

            const formData = new FormData();

            // Hanya tambahkan file ke FormData jika itu adalah Trainer DAN ada file yang dipilih
            if (user.role === 'Trainer' && file) {
            ...
            // Hanya append programGoals jika itu BUKAN Trainer
            const programGoalsElement = document.getElementById('dashboardProgramGoals');
            if (user.role !== 'Trainer' && programGoalsElement) {
                const programGoals = programGoalsElement.value.trim();
                formData.append('programGoals', programGoals);
            }

            // Hanya append description jika itu adalah Trainer
            if (user.role === 'Trainer' && description !== null) {
            ...
            if (trainerSelect) {
                ...
                if (trainerId === '' || trainerId === '0') {
                    formData.append('trainer_id', 'null');
                } else {
            ...
            .then(result => {
                if (result.success) {
                    ...
                    // Perbarui sesi dengan data pengguna terbaru setelah update
                    verifySession().then(() => {
                        // Perbarui src gambar profil jika ada path baru dari backend
                        const updatedUser = getLoggedInUser(); // Get the newly updated user from session
                        if (updatedUser && result.new_img_profile_path) {
                            updatedUser.profile = result.new_img_profile_path;
                            sessionStorage.setItem('user', encryptData(updatedUser));
                        } else if (updatedUser && !result.new_img_profile_path && user.role === 'Trainer') {
                            // If trainer updates without new image, ensure current image path is retained
                            // This might be handled by backend not returning new_img_profile_path if not uploaded
                            // but explicitly setting it can prevent accidental clears.
                            // Better: rely on verifySession to re-fetch accurate user data including profile path.
                        }
                        renderNavbar();
                        toggleSidebar();
                        navigate('home');
            ...
            .catch(error => {
                console.error('Error:', error);
                showToast('Terjadi kesalahan saat memperbarui informasi. Silakan cek konsol untuk detail.', 'danger');
            });
        }
    };
   ...
   function populateDashboard() {
        ...
        if (user) {
            ...
            // Get elements for dynamic display
            const programGoalsGroup = document.getElementById('programGoalsGroup');
            const dashboardProgramGoals = document.getElementById('dashboardProgramGoals');
            const trainerSection = document.getElementById('trainerSection');
            const trainerDescriptionGroup = document.getElementById('trainerDescriptionGroup'); // Ini adalah ID yang sebelumnya tidak konsisten, pastikan ini adalah elemen yang Anda maksud
            const profileImageGroup = document.getElementById('profileImageGroup'); // Dapatkan grup foto profil

            // Set default display states (hidden) for all dynamic sections to prevent display issues
            // Ini akan menyembunyikan semua elemen dinamis di sidebar pada awalnya.
            if (programGoalsGroup) programGoalsGroup.style.display = 'none';
            if (trainerSection) trainerSection.style.display = 'none';
            if (trainerDescriptionGroup) trainerDescriptionGroup.style.display = 'none'; // Sembunyikan secara default
            if (profileImageGroup) profileImageGroup.style.display = 'none'; // Sembunyikan secara default

            // Always update profile image source if it exists
            const profileImageExist = document.getElementById('profile_image_exist');
            if (profileImageExist) {
                profileImageExist.src = user.profile ? `./${user.profile}` : './PIC/default-profile.png'; // Fallback default image
            }


            if (user.role === 'Trainer') {
                // Program Capaian hidden for Trainers
                if (dashboardProgramGoals) {
                    dashboardProgramGoals.value = ''; // Clear value
                    dashboardProgramGoals.removeAttribute('required'); // Remove required attribute
                }

                // Trainer Description visible for Trainers
                if (trainerSection) {
                    trainerSection.style.display = 'block'; // Show trainerSection for trainers
                ...
                // Profile image upload visible for Trainers
                if (profileImageGroup) {
                    profileImageGroup.style.display = 'block';
                }

            } else if (user.role === 'Client') {
                // Program Capaian visible for Clients
                if (programGoalsGroup) {
                    programGoalsGroup.style.display = 'block';
                }
                if (dashboardProgramGoals) {
                    dashboardProgramGoals.value = user.programGoals;
                    dashboardProgramGoals.setAttribute('required', 'true'); // Add required back
                }

                // Trainer Section visible for Clients (to show assigned trainer or select one)
                if (trainerSection) {
                ...
                // Profile image upload hidden for Clients
                if (profileImageGroup) {
                    profileImageGroup.style.display = 'none';
                }

            } else if (user.role === 'Admin') {
                // Program Capaian visible for Admin
                if (programGoalsGroup) {
                    programGoalsGroup.style.display = 'block';
                }
                if (dashboardProgramGoals) {
                    dashboardProgramGoals.value = user.programGoals;
                    dashboardProgramGoals.setAttribute('required', 'true');
                }
                // Trainer Section hidden for Admin (unless specific admin-related info needs to be here)
                if (trainerSection) {
                    trainerSection.style.display = 'none';
                }
                // Profile image upload hidden for Admin
                if (profileImageGroup) {
                    profileImageGroup.style.display = 'none';
                }
            }
